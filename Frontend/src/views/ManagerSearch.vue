<template>
  <div class="manager-search">
    <div class="page-header">
      <h2><el-icon :size="22"><UserFilled /></el-icon>基金经理</h2>
    </div>

    <div class="search-bar glass-card">
      <div class="search-left">
        <el-input v-model="keyword" placeholder="输入经理姓名" style="width:280px" clearable size="large">
    ...
    <el-table :data="managers" border @selection-change="handleSelectionChange"
              :default-sort="{ prop: 'name', order: 'ascending' }" style="width:100%">
      ...
      <el-table-column prop="experienceYears" label="从业年限" width="90" align="center" sortable />
      <el-table-column prop="fundCount" label="在管基金数" width="110" align="center" sortable />
    ...
    <div class="bottom-bar glass-card" v-if="selectedManagers.length > 0">
      <el-button type="primary" @click="savePortfolio">
        <el-icon :size="15"><FolderAdd /></el-icon> 将管理基金保存为组合
      </el-button>
    </div>

    <el-dialog v-model="fundsDialogVisible" title="管理的基金" width="80%">
...
import { Search, UserFilled, FolderAdd } from '@element-plus/icons-vue'
...
const search = async () => {
  try {
    const res = await request.get('/managers', { params: { keyword: keyword.value } })
    managers.value = res.data.data.records
  } catch (e) { managers.value = [] }
}

const reset = () => { keyword.value = ''; search() }

const handleSelectionChange = (rows) => { selectedManagers.value = rows }

const showFunds = async (managerId) => {
  try {
    const res = await request.get(`/managers/${managerId}/funds`)
...
const confirmSave = async () => {
  ...
  try {
    const allFundIds = []
    for (const m of selectedManagers.value) {
      const res = await request.get(`/managers/${m.id}/funds`)
      const funds = res.data.data.records
      funds.forEach(f => allFundIds.push(f.id))
    }
    if (allFundIds.length === 0) { ElMessage.warning('所选经理没有管理基金'); return }
...
<style scoped>
.manager-search { animation: fadeInUp 0.4s ease; max-width: var(--content-max-width); margin:0 auto; }
